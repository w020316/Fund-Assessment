...
### 批次 5: 前端设计系统规划与文案交互优化

**目标**: 受硬约束限制不改 CSS/配色/布局/字体, 但需输出完整设计系统文档(供未来前端重写参考) + 可执行的文案与交互优化

#### 4.5.1 UI 设计案例调研(10 个)

从 Dribbble/Behance/站酷/GitHub 调研 10 个优质金融/数据可视化 UI 案例:

| # | 来源 | 类型 | 调研重点 |
|---|------|------|---------|
| 1 | Dribbble | 金融仪表盘 | 色彩层次、卡片设计 |
| 2 | Dribbble | 股票交易界面 | 数据可视化、图表交互 |
| 3 | Behance | 金融终端 | 深色主题、信息密度 |
| 4 | Behance | 投资分析平台 | 布局节奏、字体层级 |
| 5 | 站酷 | 量化交易UI | 中文排版、本土化 |
| 6 | 站酷 | 基金投资APP | 卡片流、操作引导 |
| 7 | GitHub | tradingview/lightweight-charts | K线图组件设计 |
| 8 | GitHub | ant-design/ant-design | 组件库规范 |
| 9 | GitHub | tailscale/golink | 深色主题设计系统 |
| 10 | GitHub | shadcn-ui/ui | 组件设计模式 |

**调研输出**: 每个案例提取设计灵感(色彩/排版/组件/交互), 形成设计灵感汇总

#### 4.5.2 设计系统文档(规划, 不立即实施)

输出完整设计系统规范文档, 包含:

**A. 色彩系统**:
- 主色(Primary): 定义 + 使用场景 + 占比规范
- 辅助色(Secondary): 定义 + 使用场景
- 中性色(Neutral): 背景层级(5级) + 文字层级(3级) + 边框/分割线
- 语义色: 涨(红)/跌(绿)/警告/成功/错误
- 暗色模式色板: 完整色值 + 对比度(WCAG AA)
...
**C. 组件设计**:
- 按钮(Primary/Secondary/Outline/Ghost + 尺寸 + 状态)
- 表单(Input/Select/Checkbox/Radio + 校验态)
- 卡片(标准/紧凑/带标题 + 阴影层级)
- 表格(斑马纹/固定头/排序态)
- 图表(折线/柱状/饼图 + 配色 + tooltip)
- 导航(侧栏/面包屑/标签页)
- 反馈(Toast/Modal/Drawer/Skeleton)
...
**E. 交互规范**:
- 动画时长: 快(150ms)/中(250ms)/慢(400ms)
- 缓动函数: ease-in-out / ease-out
- 悬浮态/按压态/禁用态
- 加载态(skeleton/spinner)

#### 4.5.3 可执行文案优化(不涉及 CSS)
...
**产出物**:
- UI设计灵感汇总(10案例)
- 设计系统规范文档(docs/2026-07-29-design-system.md)
- 文案与交互优化的代码改动(git diff)

**验收**: 设计文档完整(色彩/排版/组件/响应式/交互5部分), 无"AI"字样, 交互反馈完整
